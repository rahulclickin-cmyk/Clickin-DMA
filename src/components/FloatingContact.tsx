...
export const FloatingContact: React.FC = () => {
  ...
  const getOfferIcon = (iconName: string) => {
    switch (iconName) {
      ...
      case 'Search': return <Search className="w-4 h-4 text-amber-400 shrink-0" />;
      case 'ShieldCheck': return <ShieldCheck className="w-4 h-4 text-emerald-400 shrink-0" />;
      case 'Zap': return <Zap className="w-4 h-4 text-amber-400 shrink-0" />;
      default: return <Gift className="w-4 h-4 text-amber-400 shrink-0" />;
    }
  };
  ...
  return (
    <div className="fixed bottom-5 right-4 sm:right-6 z-50 flex flex-col items-end gap-3 select-none">
      ...
      {isAdviceWidgetOpen && (
        <div className="w-[310px] sm:w-[350px] bg-white/95 backdrop-blur-xl border border-amber-300 rounded-2xl shadow-2xl p-5 text-slate-900 space-y-4 animate-in fade-in slide-in-from-bottom-5 duration-300 relative">
          ...
          <div className="space-y-2 text-xs">
            <a
              ...
              className="block w-full text-left p-2.5 rounded-xl bg-slate-50 hover:bg-amber-50 border border-slate-200 hover:border-amber-300 transition-all font-bold text-slate-800 hover:text-amber-900 flex items-center justify-between group"
            >
          ...
          <div className="pt-2 border-t border-slate-200 flex items-center justify-between text-[11px] text-slate-600 font-medium">
            ...
            <span className="font-extrabold text-amber-900">+91 88822 92448</span>
          </div>
        </div>
      )}
      ...
      {isOfferVisible && (
        <div className="relative group max-w-[280px] sm:max-w-[320px]">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="block bg-white/95 backdrop-blur-md border border-amber-400 hover:border-amber-500 p-3.5 rounded-2xl shadow-xl transition-all duration-300 relative overflow-hidden group hover:scale-[1.02]"
          >
            {/* Animated 3-second top border countdown line */}
            <div
              key={`line-${currentOfferIndex}`}
              className="absolute top-0 left-0 h-0.5 bg-gradient-to-r from-amber-500 via-amber-400 to-amber-500 w-full animate-[shrink_3s_linear_infinite]"
            />

            <div className="flex items-start gap-3">
              <div className="p-2 bg-amber-100 border border-amber-300 rounded-xl shrink-0 mt-0.5 text-amber-800">
              ...
              <div className="space-y-0.5 min-w-0 flex-1">
                ...
                <div className="pt-1.5 flex items-center gap-1 text-[11px] font-bold text-amber-800 group-hover:underline">
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          className="bg-white hover:bg-amber-50 text-slate-900 border border-amber-300 hover:border-amber-400 px-3.5 py-2.5 rounded-full shadow-lg flex items-center gap-2 text-xs font-black cursor-pointer transition-all hover:scale-105 group"
        >
        ...
        >
          <Phone className="w-4 h-4 text-amber-600 group-hover:text-slate-950 fill-current" />
        ...
        >
          ...
          {/* Hover Tooltip */}
          <span className="absolute right-16 top-1/2 -translate-y-1/2 bg-slate-900 text-white text-xs font-bold px-3 py-1.5 rounded-xl whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none shadow-xl border border-amber-400">
