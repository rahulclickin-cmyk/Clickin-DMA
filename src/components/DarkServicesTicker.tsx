...
export const DarkServicesTicker: React.FC = () => {
  ...
  return (
    <section className="bg-slate-950 text-white py-20 px-4 md:px-8 border-t border-b border-amber-500/30 relative overflow-hidden select-none">
      
      {/* Background Decorative Tech Grid & Ambient Radial Glows */}
      <div className="absolute inset-0 bg-[radial-gradient(#fbbf24_1px,transparent_1px)] [background-size:24px_24px] opacity-[0.04] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto space-y-12 relative z-10">
        ...
        <div className="text-center space-y-4 max-w-3xl mx-auto">
          ...
          <h2 className="text-3xl md:text-5xl font-black text-white leading-tight tracking-tight">
            Full-Spectrum Digital & <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-300 via-yellow-400 to-amber-500">Search Engineering</span>
          </h2>
              ...
              <div className="flex flex-col gap-5 animate-ticker-up">
                {col1Items.map((item, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className="bg-slate-900/90 hover:bg-slate-800 border border-slate-800 hover:border-amber-400/50 rounded-2xl p-5 transition-all shadow-lg text-left space-y-3 shrink-0"
                    >
                      <div className="flex items-center justify-between">
                        <span className={`text-[10px] font-black tracking-widest uppercase px-2.5 py-1 rounded-md border ${item.color}`}>
                          {item.badge}
                        </span>
                        <span className="text-[11px] font-bold text-amber-400/90 bg-amber-400/10 px-2 py-0.5 rounded">
                          {item.highlight}
                        </span>
                      </div>
